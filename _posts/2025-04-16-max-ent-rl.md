...
#### Computing $$Z^*(s)$$ With Importance Sampling

$$
\begin{align*}
Z^*(s) &= \int_{a} \exp(Q^*(s,a)) \\
&= \int_{a} \frac{\pi^*(a \vert s)}{\pi^*(a \vert s)} \exp(Q^*(s,a)) \\
&= \mathbb{E}_{a \sim \pi*} \left[ \frac{\exp(Q^*(s,a))}{\pi^*(a \vert s)} \right]
\end{align*}
$$

Note: the sampling distribution does not have to be $$\pi^*$$ but the accuracy of the estimate relies on the sampling distribution.
